...
class Reader
{
    const STREAM_VORBIS = 1;
    const STREAM_THEORA = 2;
    const STREAM_SPEEX = 3;
    const STREAM_FLAC = 4;
    const STREAM_OPUS = 5;
     ...
     * @access  private
     */
    const STREAM_CAPTURE_VORBIS = "vorbis";

    /**
     * Capture pattern for an Ogg Speex logical stream.
     * @access  private
     */
    const STREAM_CAPTURE_SPEEX = "Speex   ";
     ...
     * @access  private
     */
    const STREAM_CAPTURE_FLAC = "FLAC";
     ...
     * @access  private
     */
    const STREAM_CAPTURE_THEORA = "theora";

    /**
     * Capture pattern for an Ogg Opus logical stream.
     * @access  private
     */
    const STREAM_CAPTURE_OPUS = "OpusHead";
    ...
    function __construct($fileLocation)
    {
        ...
        if (! file_exists($fileLocation)) {
            throw new InvalidFileException("Couldn't Open File.  Check File Path.");
        }
        ...
        if (!is_resource($this->_filePointer))
            throw new InvalidFileException("Couldn't Open File.  Check File Permissions.");

        // Check for a stream at the start
        $magic = fread($this->_filePointer, strlen(OGG_CAPTURE_PATTERN));
        if ($magic !== OGG_CAPTURE_PATTERN) {
            throw new UndecodableException("Couldn't read file: Incorrect magic number.");
        }
    ...
    static function _readBigEndian($file, $fields)
    {
        ...
        if (strlen($buffer) != $bufferLength) {
            throw new UndecodableException('Unexpected end of file');
        }
        $bytePos = 0;
        $bitPos = 0;
    ...
    function _splitStreams()
    {
        ...
        while (!feof($this->_filePointer)) {
            ...
            if ($page === false) {
                throw new UndecodableException("Cannot decode Ogg file: Invalid page at offset $this_page_offset");
            }
            ...
            if ($openStreams < 0) {
                throw new UndecodableException("Unexpected end of stream");
            }
        ...
        foreach ($this->_streamList as $stream_serial => $streamData) {
            ...
            if (preg_match("/" . STREAM_CAPTURE_VORBIS . "/", $pattern)) {
                $this->_streamList[$stream_serial]['stream_type'] = STREAM_VORBIS;
                $stream = new Stream\Vorbis($stream_serial, $streamData, $this->_filePointer);
            } elseif (preg_match("/" . STREAM_CAPTURE_SPEEX . "/", $pattern)) {
                $this->_streamList[$stream_serial]['stream_type'] = STREAM_SPEEX;
                $stream = new Stream\Speex($stream_serial, $streamData, $this->_filePointer);
            } elseif (preg_match("/" . STREAM_CAPTURE_FLAC . "/", $pattern)) {
                $this->_streamList[$stream_serial]['stream_type'] = STREAM_FLAC;
                $stream = new Stream\Flac($stream_serial, $streamData, $this->_filePointer);
            } elseif (preg_match("/" . STREAM_CAPTURE_THEORA . "/", $pattern)) {
                $this->_streamList[$stream_serial]['stream_type'] = STREAM_THEORA;
                $stream = new Stream\Theora($stream_serial, $streamData, $this->_filePointer);
            } elseif (preg_match("/" . STREAM_CAPTURE_OPUS . "/", $pattern)) {
                $this->_streamList[$stream_serial]['stream_type'] = STREAM_OPUS;
                $stream = new Stream\Opus($stream_serial, $streamData, $this->_filePointer);
            } else {
     ...
     *
     * @return File_Ogg\Stream\Bitstream
     */
    function &getStream($streamSerial)
    {
        if (! array_key_exists($streamSerial, $this->_streams))
                throw new BadSerialException("The stream number is invalid.");
    ...
    function listStreams($filter = null)
    {
        ...
        foreach ($this->_streams as $serial => $stream) {
            ...
            switch (get_class($stream)) {
                case "file_ogg\\stream\\flac":
                    $stream_type = STREAM_FLAC;
                    break;
                case "file_ogg\\stream\\speex":
                    $stream_type = STREAM_SPEEX;
                    break;
                case "file_ogg\\stream\\theora":
                    $stream_type = STREAM_THEORA;
                    break;
                case "file_ogg\\stream\\vorbis":
